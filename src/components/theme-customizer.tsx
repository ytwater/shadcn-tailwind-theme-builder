...
function Customizer() {
  ...
  const [config, setConfig] = useConfig();
  console.log("🚀 ~ file: theme-customizer.tsx:144 ~ Customizer ~ config:", config);
    ...
    <ThemeWrapper defaultTheme="zinc" className="flex flex-col space-y-4 md:space-y-6">
      <div className="flex items-start pt-4 md:pt-0">
        ...
        <Button
          ...
          onClick={() => {
            const defaultTheme = baseColors.find((theme) => theme.name === "zinc");
            setConfig({
              ...config,
              theme: "zinc",
                ...
                {baseColors.map((theme) => {
                  ...
                  return mounted ? (
                    <Button
                      ...
                      key={theme.name}
                      onClick={() => {
                        const defaultTheme = baseColors.find((t) => t.name === theme.name);
                        if (defaultTheme) {
                          setConfig({
                            ...config,
                            light: defaultTheme.cssVars.light,
            ...
            <TabsContent value="advanced">
              ...
              <div className="grid grid-cols-2 gap-2">
                {/* <Button
                  ...
                  style={
                    {
                      "--theme-primary": `hsl(${config.light.primary})`,
                    } as React.CSSProperties
                  }
                >
                  <span
                ...
                <TailwindThemeColorPicker variableName="border" />
                <TailwindThemeColorPicker variableName="input" />
  ...
  return (
    <>
      {activeTheme && (
        <Button
          onClick={() => {
            copyToClipboardWithMeta(getThemeCode(config));
            setHasCopied(true);
          }}
            ...
            {["card", "popover", "primary", "secondary", "muted", "accent", "destructive"].map(
              (prefix) => (
                <>
                  <span className="line text-white">
                    &nbsp;&nbsp;&nbsp;&nbsp;--{prefix}:{" "}
                    {config.light[prefix as keyof typeof config.light]};
                  </span>
                  <span className="line text-white">
                    &nbsp;&nbsp;&nbsp;&nbsp;--{prefix}-foreground:{" "}
                    {config.light[`${prefix}-foreground` as keyof typeof config.light]};
                  </span>
                </>
              ),
            )}
            ...
            {["card", "popover", "primary", "secondary", "muted", "accent", "destructive"].map(
              (prefix) => (
                <>
                  <span className="line text-white">
                    &nbsp;&nbsp;&nbsp;&nbsp;--{prefix}:{" "}
                    {config.dark[prefix as keyof typeof config.dark]};
                  </span>
                  <span className="line text-white">
                    &nbsp;&nbsp;&nbsp;&nbsp;--{prefix}-foreground:{" "}
                    {config.dark[`${prefix}-foreground` as keyof typeof config.dark]};
                  </span>
                </>
              ),
            )}
...
function getThemeCode(config: Config) {
  if (!config) {
    return "";
  }
